...
class CorefModel(nn.Module):
    def __init__(self, config, device, num_genres=None):
        ...
        assert config['loss_type'] in ['marginalized', 'hinge', 'partial']
    ...
    def get_predictions_and_loss(self, input_ids, input_mask, speaker_ids, sentence_len, genre, sentence_map,
                                 is_training, gold_starts=None, gold_ends=None, gold_mention_cluster_map=None, do_loss=True):
        ...
        # Get candidate span: Perform Span Enumeration
        ...
        if do_loss:
            ...
            # # Convert to golden index
            # golden_idxs = (candidate_labels > 0).nonzero(as_tuple=True)
            # temp = golden_idxs[0]
            # temp_2 = temp.tolist()
        ...
        # Extract golden spans
        # give_golden_mention = True
        if self.config['give_golden_mention']:
            # print("################ Use Golden Mention")
            candidate_idx_sorted_by_score = torch.argsort(candidate_mention_scores, descending=True).tolist()
            candidate_starts_cpu, candidate_ends_cpu = candidate_starts.tolist(), candidate_ends.tolist()
            same_start = (torch.unsqueeze(gold_starts, 1) == torch.unsqueeze(candidate_starts, 0))
            same_end = (torch.unsqueeze(gold_ends, 1) == torch.unsqueeze(candidate_ends, 0))
            ...
            # Convert to golden index
            golden_idxs = (candidate_labels > 0).nonzero(as_tuple=True)
            selected_idx_cpu = golden_idxs[0].tolist()
            # selected_idx = golden_idxs[0].tolist()
            # selected_idx = golden_idxs[0]
            # temp = golden_idxs[0]
            # temp_2 = temp.tolist()

            num_top_spans = len(selected_idx_cpu)
            # selected_idx_cpu = self._extract_top_spans(candidate_idx_sorted_by_score, candidate_starts_cpu, candidate_ends_cpu, num_top_spans)
        ...
        if not do_loss:
            ...
            return candidate_starts, candidate_ends, candidate_mention_scores, top_span_starts, top_span_ends, top_antecedent_idx, top_antecedent_scores, top_span_mention_scores
        ...
        #     loss += loss_mention
        if conf['mention_loss_coef']:
            # Add mention loss on all selected mention spans
            if conf['mention_loss_type'] == 'all':
                gold_mention_scores = top_span_mention_scores[top_span_cluster_ids > 0]
                non_gold_mention_scores = top_span_mention_scores[top_span_cluster_ids == 0]
                loss_mention = -torch.sum(torch.log(torch.sigmoid(gold_mention_scores))) * conf['mention_loss_coef']
                loss_mention += -torch.sum(torch.log(1 - torch.sigmoid(non_gold_mention_scores))) * conf['mention_loss_coef']
                loss += loss_mention
            elif conf['mention_loss_type'] == 'weighted_all':
                gold_mention_scores = top_span_mention_scores[top_span_cluster_ids > 0]
                non_gold_mention_scores = top_span_mention_scores[top_span_cluster_ids == 0]
                loss_mention = -torch.sum(torch.log(torch.sigmoid(gold_mention_scores))) * conf['mention_loss_coef']
                loss_mention += -torch.sum(torch.log(1 - torch.sigmoid(non_gold_mention_scores))) * conf[
                    'mention_loss_coef'] * float(conf['non_golden_mention_loss_weight'])
                loss += loss_mention
            # Only Add mention loss on spans that are inclued in the golden mention boundaries
            elif conf['mention_loss_type'] == 'golden':
                gold_mention_scores = top_span_mention_scores[top_span_cluster_ids > 0]
                loss_mention = -torch.sum(torch.log(torch.sigmoid(gold_mention_scores))) * conf['mention_loss_coef']
                loss += loss_mention
            # Non-Golden
            elif conf['mention_loss_type'] == 'proj_prob_aware':
                gold_mention_scores = top_span_mention_scores[top_span_cluster_ids > 0]
                non_gold_mention_scores = top_span_mention_scores[top_span_cluster_ids == 0]
                # Weight Golden Mentions with Projection Probability：
                # 1.Build Mask to map candidate mention idx to golden mention idx
                # 2.Use golden mention idx to fetch projection probability
                # same_start = (torch.unsqueeze(gold_starts, 1) == torch.unsqueeze(candidate_starts, 0))
                # same_end = (torch.unsqueeze(gold_ends, 1) == torch.unsqueeze(candidate_ends, 0))
                # same_span = (same_start & same_end).to(torch.long)
                ...
                loss_mention = -torch.sum(torch.log(torch.sigmoid(gold_mention_scores))) * conf['mention_loss_coef']
                # Weight Non Golden Mentions With Temperature Ratio
                loss_mention += -torch.sum(torch.log(1 - torch.sigmoid(non_gold_mention_scores))) * conf[
                    'mention_loss_coef'] * float(conf['non_golden_mention_loss_weight'])
                loss += loss_mention
        ...
        return [candidate_starts, candidate_ends, candidate_mention_scores, top_span_starts, top_span_ends, top_antecedent_idx, top_antecedent_scores, top_span_mention_scores], loss
    ...
    def get_predicted_antecedents(self, antecedent_idx, antecedent_scores, span_scores):
        """ CPU list input """
    ...
    def get_predicted_clusters(self, span_starts, span_ends, antecedent_idx, antecedent_scores, span_scores):
        """ CPU list input """
        # Get predicted antecedents
        predicted_antecedents = self.get_predicted_antecedents(antecedent_idx, antecedent_scores, span_scores)
        ...
        for i, predicted_idx in enumerate(predicted_antecedents):
            ...
            if predicted_idx == -2:
                # print("Should Add Singleton")
                antecedent = (int(span_starts[i]), int(span_ends[i]))
                # antecedent = (int(span_starts[predicted_idx]), int(span_ends[predicted_idx]))
                if antecedent not in mention_to_cluster_id:
                    # print("Antecedent is not in mention_to_cluster_id")
                    antecedent_cluster_id = len(predicted_clusters)
                    predicted_clusters.append([antecedent])
                    mention_to_cluster_id[antecedent] = antecedent_cluster_id
    ...
    def update_evaluator(self, span_starts, span_ends, antecedent_idx, antecedent_scores, gold_clusters, span_scores, evaluator, mention_evaluator):
        predicted_clusters, mention_to_cluster_id, _ = self.get_predicted_clusters(span_starts, span_ends, antecedent_idx, antecedent_scores, span_scores)
    ...
    # def update_evaluator_prediction(self, span_starts, span_ends, antecedent_idx, antecedent_scores, gold_clusters, span_scores, evaluator, mention_evaluator):
    #     predicted_clusters, mention_to_cluster_id, _ = self.get_predicted_clusters(span_starts, span_ends, antecedent_idx, antecedent_scores, span_scores)
    #     mention_to_predicted = {m: predicted_clusters[cluster_idx] for m, cluster_idx in mention_to_cluster_id.items()}
    #     gold_clusters = [tuple(tuple(m) for m in cluster) for cluster in gold_clusters]
    #     mention_to_gold = {m: cluster for cluster in gold_clusters for m in cluster}
    ...
    #     }
    #     return predicted_clusters, predictions
